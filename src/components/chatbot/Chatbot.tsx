import type { ChatMessage } from "#client/api/public/chatbot";
...
import { sendChatMessage } from "#client/api/public/chatbot";
...
export function Chatbot() {
  ...
  const canUseChatbot = !user || (user && user.accountType === "student");
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const isStatusQuery =
        /status|application|applied|my.*application|check.*application|application.*status/i.test(
          messageLower,
        );

      if (isStatusQuery && user) {
        try {
          const applications = await apiGet<MyAppRow[]>(
          ...
          assistantMessage.content =
            "I couldn't fetch your applications right now. Please try checking your dashboard.";
          assistantMessage.actions = [
      ...
      if (
        (messageLower.includes("sign up") ||
          ...
          messageLower.includes("create account")) &&
        !user
      ) {
        assistantMessage.actions = [
      ...
      const isProjectSearch =
        /find|show|search|looking for|recommend|suggest|want.*project|need.*project|any.*project|available.*project|environmental|community|mentoring|elderly|animal|arts|culture|sports|coding/i.test(
          messageLower,
        );
      ...
      if (isProjectSearch) {
        try {
          ...
          const searchTerms =
            messageLower.match(
              /(environmental|community|mentoring|elderly|animal|arts|culture|sports|coding|local|overseas)/gi,
            ) || [];
          ...
          if (filteredProjects.length > 0) {
            assistantMessage.projects = filteredProjects
              .slice(0, 3)
              .map((p: any) => ({
                id: p.id,
                title: p.title,
    ...
    <>
      ...
      >
        <Button
          ...
          className={`relative h-14 overflow-hidden rounded-full shadow-lg transition-all hover:shadow-xl ${
            isHovered && !isOpen && !isMobileOrTablet
              ? "text-primary border-2 border-white bg-white hover:bg-white"
              : ""
          }`}
          style={{
            width:
              isHovered && !isOpen && !isMobileOrTablet ? "auto" : "3.5rem",
            minWidth: "3.5rem",
            paddingLeft:
              isHovered && !isOpen && !isMobileOrTablet ? "1rem" : "0",
            paddingRight:
              isHovered && !isOpen && !isMobileOrTablet ? "1rem" : "0",
            backgroundColor:
              isHovered && !isOpen && !isMobileOrTablet ? "#ffffff" : undefined,
          }}
        >
          ...
          {isHovered && !isOpen && !isMobileOrTablet && (
            <motion.div
              className="from-primary/30 via-accent/30 to-secondary/30 absolute inset-0 rounded-full bg-gradient-to-r"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
          ...
          {isHovered && !isOpen && !isMobileOrTablet && (
            <motion.div
              className="from-primary/20 via-accent/20 to-secondary/20 absolute inset-0 -z-10 rounded-full bg-gradient-to-r blur-xl"
              initial={{ opacity: 0, scale: 0.8 }}
        ...
        {isOpen && (
          <motion.div
            ...
            className={`fixed z-50 shadow-2xl ${
              isMobileOrTablet
                ? "right-0 bottom-0 left-0 h-[80vh] max-h-[80vh] w-full rounded-t-2xl"
                : "right-6 bottom-24 h-[480px] max-h-[calc(100vh-10rem)] w-96 max-w-[calc(100vw-3rem)] lg:max-w-none"
            }`}
            style={isMobileOrTablet ? {} : { originX: 1, originY: 1 }}
          >
            <Card
              className="flex h-full w-full flex-col overflow-hidden !py-0"
            ...
            >
                  ...
                  {messagesState.length === 1 && showSuggestions && (
                    <div className="text-center">
                      <p className="text-muted-foreground text-xs">
                        Try one of the suggested questions below, or type your
                        own!
                      </p>
                    </div>
                  )}
                          ...
                          <div className="mt-2 max-w-[80%] space-y-2">
                                ...
                                >
                                  <div className="bg-background border-border hover:bg-muted/50 cursor-pointer rounded-lg border p-3 transition-colors">
                                    <div className="flex items-start justify-between gap-2">
                                      ...
                                      <Badge
                                        className={`border text-xs ${getStatusColor(app.status)}`}
                                      >
                            ...
                            {message.applications.length > 5 && (
                              <p className="text-muted-foreground text-center text-xs">
                                +{message.applications.length - 5} more
                                application
                                {message.applications.length - 5 !== 1
                                  ? "s"
                                  : ""}
                              </p>
                            )}
              ...
              {showSuggestions && messagesState.length <= 1 && (
                <div
                  className="overflow-hidden border-t px-4 pt-2"
                  ...
                  >
                    {suggestedQuestions.map((question, index) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="h-auto px-2.5 py-1 text-xs break-words whitespace-normal"
              ...
              <div className="overflow-hidden border-t px-4 pt-3 pb-4">
