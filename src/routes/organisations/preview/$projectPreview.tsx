...
export const Route = createFileRoute("/organisations/preview/$projectPreview")({
  component: CspDetail,
});
...
const [showLoginModal, setShowLoginModal] = useState(false);
  ...
  const handlefavourite = () => {
    ...
    // Favourite functionality disabled for organisation preview
  };

  const handleShare = async () => {
    // Share functionality disabled for organisation preview
  };
  ...
  const { projectPreview } = Route.useParams();

  const { data: csp, isLoading, isError } = useQuery({
    queryKey: ["csp-detail", projectPreview],
    queryFn: () => fetchCspById(projectPreview),
  });
          ...
          <div className="fixed bottom-6 right-6 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <Dialog open={showApplicationDialog} onOpenChange={setShowApplicationDialog}>
              <DialogTrigger asChild>
                <Button size="lg" className="shadow-2xl hover:shadow-xl transition-shadow" disabled>
