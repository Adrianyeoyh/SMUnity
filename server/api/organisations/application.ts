...
application.patch("/decision", async (c) => {
  try {
    ...
    if (!application) {
      console.warn(` [Application Decision] Application ${applicationId} not found`);
      return c.json({ error: "Application not found" }, 404);
    }

    if (application.orgId !== orgUser.id) {
      console.warn(` [Application Decision] Org ${orgUser.id} tried to modify application not theirs`);
